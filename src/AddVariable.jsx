...
function AddVariable() {
  ...
  const handleAddVariable = () => {
    const currentVariables = Array.from(variables).map(v => parseInt(v.replace(/[{}]/g, ''), 10));
  ...
  return (
    <Box component="form" mt={3} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }} noValidate autoComplete="off" fullWidth>
      <TextField
        label="Body"
        placeholder="Hello"
        variant="outlined"
      ...
      <Box mb={2} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <WhiteButton variant="contained" startIcon={<AddIcon fontSize="small" />} onClick={handleAddVariable}>
          Add Variable
        </WhiteButton>
      </Box>
      ...
      <Box 
        padding={1}
        sx={{ backgroundColor: "lightgray", borderRadius: "3px", width:"860px"  }}
        >
            <Typography variant="h6" align="left"> Samples for body content</Typography>
            <Typography variant="body2" align="left"> To help us review your message template, please add an example for each variable in your body text. Do not use real customer information. Could API hosted by Meta reviews templates and varible parameters to protect the security oand integrity of our services</Typography>
